                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php $no = 1;
                                foreach ($AmbilSemuaDataPekerjaan as $key => $value) { ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><?= $value['nama_pekerjaan'] ?></td>
                                        <td>
                                            <button class="btn btn-warning btn-sm" data-toggle="modal" data-target="#ubah<?= $value['id_pekerjaan'] ?>">
                                                <i class="fas fa-edit"></i> Ubah
                                            </button>
                                            <button class="btn btn-danger btn-sm" data-toggle="modal" data-target="#hapus<?= $value['id_pekerjaan'] ?>">
...
<div class="modal fade" id="tambah">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <?php echo form_open('Superadmin/TambahDataPekerjaan') ?>
            <?= csrf_field() ?>
            <div class="modal-body">
                <div class="form-group">
                    <label>Nama <?= $subtitle; ?></label>
                    <input name="nama_pekerjaan" class="form-control" placeholder="Nama <?= $subtitle; ?>" required>
                </div>
            </div>
...
<!-- Modal Ubah -->
<?php foreach ($AmbilSemuaDataPekerjaan as $key => $value) { ?>
    <div class="modal fade" id="ubah<?= $value['id_pekerjaan'] ?>">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <?php echo form_open('Superadmin/UbahDataPekerjaan/' . $value['id_pekerjaan']) ?>
                <?= csrf_field() ?>
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama <?= $subtitle; ?></label>
                        <input name="nama_pekerjaan" value="<?= $value['nama_pekerjaan'] ?>" class="form-control" placeholder="Nama <?= $subtitle; ?>" required>
                    </div>
                </div>
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-outline-danger btn-sm" data-dismiss="modal">Batal</button>
    ...
    <div class="modal fade" id="hapus<?= $value['id_pekerjaan'] ?>">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Apakah Anda Ingin Menghapus <b><?= $subtitle; ?> <?= $value['nama_pekerjaan'] ?></b> ?
                </div>
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-outline-danger btn-sm" data-dismiss="modal">Batal</button>
                    <a href="<?= base_url('superadmin/HapusDataPekerjaan/' . $value['id_pekerjaan']) ?>" class="btn btn-primary btn-sm">Hapus</a>
                </div>
            </div>
        </div>
    </div>
