                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php $no = 1;
                                foreach ($AmbilSemuaDataKelas as $key => $value) { ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><?= $value['nama_kelas'] ?></td>
                                        <td>
                                            <button class="btn btn-warning btn-sm" data-toggle="modal" data-target="#ubah<?= $value['id_kelas'] ?>">
                                                <i class="fas fa-edit"></i> Ubah
                                            </button>
                                            <button class="btn btn-danger btn-sm" data-toggle="modal" data-target="#hapus<?= $value['id_kelas'] ?>">
...
<div class="modal fade" id="tambah">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <?php echo form_open('Superadmin/TambahDataKelas') ?>
            <?= csrf_field() ?>
...
<!-- Modal Ubah -->
<?php foreach ($AmbilSemuaDataKelas as $key => $value) { ?>
    <div class="modal fade" id="ubah<?= $value['id_kelas'] ?>">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <?php echo form_open('Superadmin/UbahDataKelas/' . $value['id_kelas']) ?>
                <?= csrf_field() ?>
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama <?= $subtitle; ?></label>
                        <input name="nama_kelas" value="<?= $value['nama_kelas'] ?>" class="form-control" placeholder="Nama Kelas" required>
                    </div>
                </div>
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-outline-danger btn-sm" data-dismiss="modal">Batal</button>
...
<!-- Modal Hapus -->
<?php foreach ($AmbilSemuaDataKelas as $key => $value) { ?>
    <div class="modal fade" id="hapus<?= $value['id_kelas'] ?>">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    Apakah Anda Ingin Menghapus <b><?= $subtitle; ?> <?= $value['nama_kelas'] ?></b> ?
                </div>
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-outline-danger btn-sm" data-dismiss="modal">Batal</button>
                    <a href="<?= base_url('superadmin/HapusDataKelas/' . $value['id_kelas']) ?>" class="btn btn-primary btn-sm">Hapus</a>
                </div>
            </div>
        </div>
    </div>
